...
./build-emacs-for-macos --native-comp emacs-29.1
[[https://git.savannah.gnu.org/cgit/emacs.git][Emacs branches]]

Newest way to install emacs:
brew tap d12frosted/emacs-plus
brew install emacs-plus@29 --with-native-comp --with-xwidgets --with-imagemagick
brew install emacs-plus@30 --with-native-comp --with-xwidgets --with-imagemagick --with-poll
...
;;  (setq gc-cons-threshold 100000000)
  ...
  (global-set-key (kbd "M-w") 'sd/copy-line)
...
;;  (global-set-key (kbd "M-v") 'xah-paste-from-register-1)
  ...
  ;; don't put any text in the scratch buffer
  ...
  (global-set-key (kbd "C-<f8>") 'sd/open-org-init)

  ;; open bookmarks
  (global-set-key (kbd "<f8>") 'sd/open-bookmarks)
  ...
  (global-set-key (kbd "C-M-i") 'sd/vc-grep)
  ...
  ;; conveniently map repeat to F7 and M-.
  ...
  ;; mark to the beginning of line
  (global-unset-key (kbd "M--"))
  (global-set-key (kbd "M--") 'sd/mark-from-point-to-beginning-of-line)
...
;  (global-set-key (kbd "M-=") 'sd/mark-line)
  ...
  (global-set-key (kbd "S-SPC") 'consult-goto-line)
  ...
  (global-set-key [f11] 'sd/bbg-open)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun sd/open-bookmarks()
    (interactive)
    "Open initialization file and move to the end of the buffer."
    (find-file "~/notes/bookmarks.org"))
  ...
  (defun sd/mark-from-point-to-beginning-of-line ()
  ...
  (defun sd/minibuffer-setup ()
    (set (make-local-variable 'face-remapping-alist)
         '((default :background "black" :height 1.5))))

  (defun sd/copy-line (arg)
  ...
  (defun sd/sort-words-in-region (beg end &optional reversed)
  ...
  (defun sd/vc-grep ()
    "Use counsel-rg for non-git repositories.
     Use counsel-git-grep on git repositories"
    (interactive)
    (if (memq (vc-backend (buffer-file-name)) '(SVN CVS RCS SCCS))
        (counsel-projectile-rg)
      (counsel-git-grep)))

  (defun sd/region-to-string (beginning end)
    (interactive "r")
    (let* ((selection (buffer-substring-no-properties beginning end)))
          (delete-region beginning end)
          (insert (prin1-to-string selection))))

  (defun sd/bbg-open()
    (interactive)
    (shell-command (format "open \"bbg://screens/%s\""
  ...
  (defun chrome-bookmarks-insert-as-org ()
    ...
    (defun sd/roam-commit-and-push ()
      "Commit and push staged files with a comment of `Roam`"
      (interactive)
      (magit-commit-create (list "-m" "Roam"))
      (call-interactively #'magit-push-current-to-pushremote))

    (defun sd/roam-stage-commit-and-push ()
      "Commit and push staged files with a comment of `Roam`"
      (interactive)
      (call-interactively 'magit-stage-buffer-file)
      (magit-commit-create (list "-m" "Roam"))
      (call-interactively #'magit-push-current-to-pushremote))
...
#+BEGIN_SRC emacs-lisp
  ...
  (set-face-background 'fringe "gray11")
  ;; color of comments
...
#+BEGIN_SRC emacs-lisp
  ...
  (defalias '\"  'sd/region-to-string)
  (defalias 's   'sort-lines)
  (defalias 'n   'narrow-to-defun)
  (defalias 'w   'widen)
  (defalias 'ms 'magit-stage-buffer-file)
#+END_SRC
* Modes
** 0xc
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package beacon
    ...
    (setq beacon-blink-duration 0.01))
#+END_SRC
** calc
#+begin_src emacs-lisp
  (use-package calc
    :config
    (setq calc-window-height 25)
    (unbind-key "M-k" calc-mode-map))
#+end_src
** cc-mode
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'auto-mode-alist '("\\.[hcp]\\'" . c++-mode))
  ...
  (use-package cc-mode
    ...
    :bind(
          :map c++-mode-map
               ...
               ("M-m"   . counsel-imenu)
               ("<return>" . sd/newline)
               ( "M-<right>" . sd/comment-line-or-region)
  ...
  (setq c-default-style
    ...
    (set-face-attribute 'font-lock-preprocessor-face nil :background "gray21" :foreground "green" :box '(:line-width 2 :color "dark cyan") :weight 'normal)
    ;; color BSLS_PLATFORM_OS_LINUX as a type
    (font-lock-add-keywords 'c++-mode
                        '(("BSLS_PLATFORM_OS_LINUX" . 'font-lock-type-face)))
...
#+BEGIN_SRC emacs-lisp
  (use-package ccls
    ...
    ;; copy of function from ccls package, so that root is correctly determined for c++-ts-mode c-ts-mode modes
    (defun ccls--suggest-project-root ()
      (and (memq major-mode '(c-mode c++-mode cuda-mode objc-mode c++-ts-mode c-ts-mode))
    ...
    (setq ccls-args '("--log-file=/Users/sdayts/logs/ccls.log"))
    ;; syntax highlight code surrounded by preprocessor
...
#+BEGIN_SRC emacs-lisp
  (use-package company
    ...
    (setq company-tooltip-minimum 20)
    (custom-set-faces
     '(company-preview
...
#+BEGIN_SRC emacs-lisp
    (use-package conf-mode
      :mode (("\\.pkgcfg\\'"     . conf-mode)
             ("\\.sql\\'"        . conf-mode)
             ("\\.gitignore\\'"  . conf-mode)
             ("\\Jenkinsfile\\'" . conf-mode)
             ("\\.rc\\'"         . conf-mode)
             ("control\\'"       . conf-mode)
             ("\\.cap\\'"        . conf-mode)
             ("\\.defs\\'"       . conf-mode)
...
#+BEGIN_SRC emacs-lisp
  (use-package git-gutter+
    :ensure    nil
    :after magit
    :load-path "~/.emacs.d/local-packages/git-gutter-plus")
#+END_SRC
** git-gutter-fringe+
#+BEGIN_SRC emacs-lisp
  (use-package git-gutter-fringe+
    :ensure    nil
    :load-path "~/.emacs.d/local-packages/git-gutter-fringe-plus"
    :after git-gutter+
...
#+BEGIN_SRC emacs-lisp
  ...
  (defhydra sd/hydra-main (:color pink
                           :hint nil)
    "
  ^Places^                        ^Shortcuts^       ^Actions^                   ^Bookmarks^              ^Hydras^
  ^^^^^^^^--------------------------------------------------------------------------------------------------------------------------
  _a_: Agenda                     _m_: Magit        _k_: Kill current buffer    _j_: Jira                _b_: Buffer Hydra
  _A_: Display Agenda             _v_: Vterm                                  _t_: Triage Schedule     _M_: Magit Hydra
  _I_: Init Configuration File    _d_: Dired
  _s_: Scratch Buffer             _i_: IBuffer
                                _x_: Hex <-> Dec
                                _r_: Push Roam
                                _u_: Open Url

  "
    ("a" (find-file "~/notes/agenda.org")                     :color blue)
    ("A" org-agenda-list                                      :color blue)
    ("I" (find-file "~/.emacs.d.org/myinit.org")              :color blue)
    ("m" magit-status                                         :color blue)
    ("v" vterm                                                :color blue)
    ("d" dired-jump                                           :color blue)
    ("i" ibuffer                                              :color blue)
    ("x" 0xc-live-convert                                     :color blue)
    ("r" sd/roam-stage-commit-and-push                        :color blue)
    ("k" kill-current-buffer                                  :color blue)
    ("s" sd/switch-to-scratch                                 :color blue)
    ("j" (browse-url "https://bburl/rurKC")                   :color blue)
    ("b" sd/hydra-buffer/body                                 :color blue)
    ("t" (browse-url "https://bburl/iNG7J")                   :color blue)
    ("u" browse-url-at-point                                  :color blue)
    ("M" sd/hydra-magit/body                                  :color blue)
    ("q" nil "quit"))

  (global-set-key (kbd "M-SPC") 'sd/hydra-main/body)

  (defhydra sd/hydra-buffer (:color pink
                           :hint nil)
    "
  ^Buffer^
  ^^^^^^^^------------------------
  ...
  (defhydra sd/hydra-magit (:color pink
                           :hint nil)
    "
  ^Magit^
  ^^^^^^^^-----------------------------
  _d_: Diff Current Buffer/File
  _h_: File history
  _l_: Magit Log Current
  _s_: Stage Buffer File

  "
    ("d" magit-diff-buffer-file :color blue)
    ("h" magit-log-buffer-file :color blue)
    ("s" magit-stage-buffer-file :color blue)
    ("l" magit-log-current :color blue)
    ("q" nil "quit"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package ivy
    ...
    ;; remove annoying ^ prefix
    (setq ivy-initial-inputs-alist nil)
    (set-face-attribute 'ivy-current-match nil :background "RoyalBlue4" :foreground "yellow" :weight 'normal  :box '(:line-width 1 :color "RoyalBlue4"))
    ...
    (set-face-attribute 'ivy-minibuffer-match-highlight nil :background "black" :foreground "#FFF8DC"  :weight 'normal)
    (ivy-mode 1))
#+END_SRC
** ivy-posframe
#+BEGIN_SRC emacs-lisp
    ...
    (use-package org
      ...
      (setq org-todo-keyword-faces
            '(("IN-PROGRESS"          . "yellow")
              ("BLOCKED"              . "red")
              ("UNUSED"               . "green")
              ("IN-TESTING"           . "systemOrangeColor")
              ("RELESE-IN-PROGRESS"   . "cyan")))
      ;; this fixes an issue with [[https://github.com/abo-abo/swiper/issues/3015][org mode overlays]]
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-x b") 'tab-bar-new-tab)
...
;  (venv-workon "p3")
  ...
  (setq lsp-python-executable-cmd "python3.13")
